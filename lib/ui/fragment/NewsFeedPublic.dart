...
class NewsFeedPublic extends StatefulWidget {
  const NewsFeedPublic({Key key}) : super(key: key);

  @override
  _NewsFeedPublicState createState() => _NewsFeedPublicState();
}

class _NewsFeedPublicState extends State<NewsFeedPublic>
                                                  ...
                                                  //  SizedBox(width: 10,),
                                                    feed.commentCount != 0
                                                      ?AppHelper.countWidget(
                                                          feed.commentCount):SizedBox(width: 3,),
                                                    GestureDetector(
                                                      onTap: () {
  ...
  getFeedAPI() {
    ...
    ApiRequest(
      ...
      url: Url.publicFees,
      body: body,
      apiAction: ApiAction.publicFees,
    );
  }
  ...
  getSearchData() {
    ...
    if (search.isEmpty) {
      ...
      Map<String, String> body = Map();
      body['save_as'] = "3"; //3 = public
      body['keyword'] = search; //1 = public
      ApiRequest(
          context: context,
          apiCallBackListener: this,
          showLoader: false,
  ...
  apiCallBackListener(String action, result) {
    if (action == ApiAction.publicFees) {
