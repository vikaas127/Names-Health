...
class PostSignScreen extends StatefulWidget {
  Feed feed;

  PostSignScreen({Key key, this.feed}) : super(key: key);

  @override
  State<PostSignScreen> createState() => _PostSignScreenState();
}

class _PostSignScreenState extends State<PostSignScreen>
                ...
                child: FutureBuilder<LikedUserModel>(
                    ...
                    builder: (BuildContext context,
                      ...
                      if (snapshot.hasData) {
                        print(
                            "-----------------------data-----------------------------");
                        print(likedUserModel.data.list.length);
                        print(likedUserModel.data.list);
  ...
  @override
  void initState() {
    futureData = getSignedUserAPI();
    super.initState();
  }

  @override
  apiCallBackListener(String action, result) {
    if (action == ApiAction.signUserList) {
  ...
  Future<void> getSignedUserAPI() {
    ...
    ApiRequest(
        ...
        showLoader: false,
        url: Url.signUserList,
        apiAction: ApiAction.signUserList,
        body: body);
  }
  ...
  String _getTitle() {
    if (widget.feed.likedCount == 0) {
      ...
      return "Signed (" + widget.feed.signedCount.toString() + ")";
    }
    return "Signed (" + widget.feed.signedCount.toString() + ")";
  }
}
